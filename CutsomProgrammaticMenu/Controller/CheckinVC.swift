...
class CheckinVC: BaseMenuVC {
    ...
    fileprivate func addMapView()
    {
        ...
        mapView.anchorToView(top: view.safeAreaLayoutGuide.topAnchor, leading: view.leadingAnchor, bottom: view.safeAreaLayoutGuide.bottomAnchor, trailing: view.trailingAnchor,padding: .init(top: 50, left: 0, bottom: 0, right: 0))
    }
...
extension CheckinVC:CLLocationManagerDelegate
{
    ...
    fileprivate func getAttendanceFromAPI()
    {
        ...
        guard let userID = NewUser.getLocallySavedUser()?.employeeId else {return}
        let url = Utility.getFinalURL(methodName: "GetEmpExternalSites");
        let  finalURL = "\(url)?empId=1&isEng=true"
        
        let header = ["Authorization":"Bearer \(NewUser.getLocallySavedUser()?.token ?? "")","Content-Type":"application/json; charset=utf-8"]
        print(finalURL)
        Service.shared.fetchGenericJSONData(urlString: finalURL, parameters: nil,header:header) {[weak self] (res:[UserDestinations]?, err) in
...
//                ProgressHUD.showError(err!.localizedDescription)
    ...
    fileprivate func checkingDistance()
    {
        ...
        userDestinations.forEach { (dest) in
            ...
            else if dest.latitude == closest.coordinate.latitude && dest.longitude == closest.coordinate.longitude
            {
                ...
                self.closestLocation = dest
                if distanceInMeters > Double(dest.range)
                {
                    ProgressHUD.dismiss()
    ...
    fileprivate func checkUserViaAPI(transationType:Int = 1)
    {
        ...
        guard let currentUserID = NewUser.getLocallySavedUser()?.employeeId   else{return}
