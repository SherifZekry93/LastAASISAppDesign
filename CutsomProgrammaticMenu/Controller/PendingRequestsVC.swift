...
class PendingRequestsVC: BaseMenuVC {
    ...
    func setupHorizontalMenuBar()
    {
        ...
        horizontalBarView.anchorToView(top: nil, leading: menuCollectionView.leadingAnchor, bottom: menuCollectionView.bottomAnchor, trailing: nil,padding: .init(top: 0, left: 0, bottom: 0, right: 0),size:.init(width: view.frame.width / 2, height: 2.5))
    ...
    private func setupTitleView()
    {
        ...
        labelTitle.text = "Pending Requests".localized()
        labelTitle.textColor = .white
        addSubview(labelTitle)
        labelTitle.anchorToView(top: view.safeAreaLayoutGuide.topAnchor, leading: view.leadingAnchor, bottom: nil
            , trailing: view.trailingAnchor,padding: .init(top: 15, left: 0, bottom: 0, right: 0))
...
extension PendingRequestsVC:UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UICollectionViewDelegate
{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == menuCollectionView
        {
            return .init(width: view.frame.width / 2, height: 50)
        }
        else
        {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == menuCollectionView
        {
            let distance = view.frame.width / 2
    ...
    private func indexChanged(index:Int)
    {
        switch index {
        case 0:
            let url = Utility.getFinalURL(methodName: "GetRequests");
            
            let params:[String:Any] =
            [
                "requestTypeId": 0,
                "requestStatus": 0,
                "fromDateSubmitting": "1/1/2017",
                "toDateSubmitting": "1/31/2019",
                "pageIndex": 0,
                "pageSize": 0,
                "isEng": true
            ]
            ...
            Service.shared.fetchGenericJSONData(methodType: .post, urlString: url, parameters: params, header: header) { (result:PendingRequestModel?, err:Error?) in
                  print(err)
                  print(result)
            }
        case 1:
            print("get Permissoins")
        default:
            print("2")
        }
    }
