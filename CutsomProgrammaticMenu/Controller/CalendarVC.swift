...
class CalendarVC:BaseMenuVC
{
    ...
    let absentLabel:UILabel = UILabel();
    
    let attendLabel:UILabel = UILabel();
    
    let weekendLabel:UILabel = UILabel();
    
    let publicHolidayLabel:UILabel = UILabel();
    
    let privateHolidayLabel:UILabel = UILabel();
    ...
    lazy var calendarStatisticsView:UIStackView  = {
        ...
        let firstStack = UIStackView(arrangedSubviews: [
            getStackStatistics(txtColor: .redMessageColor, statString: "Absent", statNumber: absentLabel),
            getStackStatistics(txtColor: .greenMessageColor, statString: "Attend", statNumber: attendLabel),
            getStackStatistics(txtColor: .statisticsOrangeText, statString: "Weekend", statNumber: weekendLabel)
        ])
        ...
        let secondStack = UIStackView(arrangedSubviews: [
            getStackStatistics(txtColor: .blueCircleText, statString: "Public-Holiday", statNumber: publicHolidayLabel),
            getStackStatistics(txtColor: .headerControlllerBlue, statString: "Private-Holiday", statNumber: privateHolidayLabel),
            ])
    ...
    private func getStackStatistics(txtColor:UIColor,statString:String,statNumber:UILabel) -> UIView
    {
        ...
        attributed.append(NSAttributedString(string: "\(0)", attributes: [
            NSAttributedString.Key.font:UIFont.boldSystemFont(ofSize: 30),
            NSAttributedString.Key.foregroundColor:txtColor
        ]))
        
        statNumber.numberOfLines = -1
        
        statNumber.attributedText = attributed

        statNumber.textAlignment = .center
        
        statNumber.backgroundColor = .white
    ...
    fileprivate func getCurrentEmployeeAttendance()
    {
         ...
         do
        {
            ...
            result?.forEach({ (attend) in
                ...
                self.changeLabelCountText(txtColor: .redMessageColor, statString: "Absent", statNumber: self.absecntDays.count, label: self.absentLabel)
                
                self.changeLabelCountText(txtColor: .greenMessageColor, statString: "Attend", statNumber: self.attendedDays.count, label: self.attendLabel)
                
                self.changeLabelCountText(txtColor: .statisticsOrangeText, statString: "Weekend", statNumber: self.weekEndDays.count, label: self.weekendLabel)
                
                self.changeLabelCountText(txtColor: .blueCircleText, statString: "Public-Holiday", statNumber: self.publicHoliday.count, label: self.publicHolidayLabel)
                self.changeLabelCountText(txtColor: .headerControlllerBlue, statString: "Private-Holiday", statNumber: self.privateHoliday.count, label: self.privateHolidayLabel)
    ...
    private func changeLabelCountText(txtColor:UIColor,statString:String,statNumber:Int,label:UILabel)
    {
...
extension CalendarVC:FSCalendarDataSource, FSCalendarDelegate,FSCalendarDelegateAppearance
{
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, fillDefaultColorFor date: Date) -> UIColor? {
        ...
        {
            return .redMessageColor
        }
        else if weekEndDays.contains(dateString)
        {
            return .statisticsOrangeText//UIColor.blue
        }
        else if privateHoliday.contains(dateString)
        {
            return .headerControlllerBlue//UIColor(red: 204/255, green: 204/255, blue: 0/255, alpha: 1)
        }
        else if publicHoliday.contains(dateString)
        {
            return .blueCircleText//.purple
        }
        else if attendedDays.contains(dateString)
        {
            return .greenMessageColor//UIColor(red: 0/255, green: 100/255, blue: 0/255, alpha: 1)
        }
        else{
            return nil
        }
